...
/*
Release R5.0 - modified the charge adjustments in CAN data - V4 cause OV-ILLC errors???
updated 9/4/2025
...
int CAN_Adjusted_Battery_discharge_current_limit;  // modified charge current value, use to create an enhanced & personalised charging and battery care strategy...i.e reduce charge rate at high delta V or towards temperature extremes   charging current as part of a in mA/100  0,0 - 500,0 A   ie 550 = 55000mA = 55A
float Hysterisis=0;



/*Decisions made from the most stringent limit downwards
  If max Cell V >3.5  Stop Charging ! 
  If Max Cell V >3.4 (3.45??) OR DeltaV > 0.1V ,  Limit to 2A 
  If Ave Cell V >3.35V (approx 90%) OR DeltaV > 80mV limit to 10A //Serial.println(Delta_Cell_Voltage);  Cell_MaxV
  else, no problem we use the default set limit 50A
*/

// Arrays for thresholds and corresponding current steps
const int NUM_STEPS = 4;
float AveCellVThresholds[NUM_STEPS] = { 3.300, 3.350, 3.400, 3.480 };  // Voltage thresholds (in volts)
float Hysteresis[NUM_STEPS] = { 0.04, 0.03, 0.03, 0.03 };              // Hysteresis for each threshold
int CAN_ChgI_Limits[NUM_STEPS] = { 300, 150, 20, 0 };                  // Adjusted Charge Current Limits (in amps x10)
...
bool Halt_Discharge =false;    //when we detect or reach 20% SOC we will stop the Inverter discharging battery 
bool Force_Charge =false;    //when we detect a very low battery state we Gorce the Inverter to charge the  battery 
...
void setup() {
  ...
  //ledcSetup(0, 10000, 8);  deprocated
  ledcAttach(38,10000,8);  //pin 38 controlls Backlight on T-Display S3, High is 255 = full On,  This pin is on Channel 0
  ledcWrite(38, 254);     // For some reason dimming down from 255 to 254..253 etc causes a jump in brightness..DC to a 10kHz change?!?
...
void loop() {
  ...
  if (ble_connected) {
    ...
    if ((canpublishtime == 0) || (millis() >= (canpublishtime + CAN_Update_Interval))) {
      ...
      if (CAN_use) {
        CAN_Charge_Current_Adjust();    //We look for High or Low Charging conditions an manage charge and discharge current levels accordingly
        CAN_Low_Charge_SOC_Control();   //Maybe these could be timed to update every n seconds
