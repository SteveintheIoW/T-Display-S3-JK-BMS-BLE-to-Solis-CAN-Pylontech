...
void Display_To_Sleep(void) {
  ...
  for (int i = 254; i >= 0; i--) {  //255 = full brightness, 0 = Backlight Off
    //ledcSetup(0, 10000, 8);
    ledcAttach(38,10000,8);
    ledcWrite(38, i);  // PWM steadily reduced to dim the backlight
    delay(10);
  }
...
void BootSplashScreen(void) {
  ...
  sprite.setTextColor(TFT_WHITE);
  sprite.drawString("Steve T. IoW 2024 Firmware Ver ", 30, 33, 2);
  sprite.drawString(SW_Version, 245, 33, 2);
...
void CAN_Display(void) {  //Pressing RHS button shows this individual cell Voltage display
  ...
  if (CAN_Adjusted_Battery_discharge_current_limit != CAN_Battery_discharge_current_limit) {

    sprite.setTextColor(TFT_RED, TFT_BLACK);
    sprite.drawString("Adj. Discharge Current", 2, 115, 2);
    sprite.drawString(String(float(CAN_Adjusted_Battery_discharge_current_limit) / 10.0, 1) + "A", 155, 115, 2);
    sprite.setTextColor(TFT_YELLOW, TFT_BLACK);
    sprite.drawString("(Default " + String(float(CAN_Battery_discharge_current_limit / 10.0), 1) + "A)", 200, 115, 2);
